...
def _run_training(req: TrainRequest):
    ...
    model = NeuralNetwork.from_config(req.network_config)
    trainer = Trainer()
    ...
    result = trainer.train(
        ...
        regularization=req.network_config.regularization,
        reg_lambda=req.network_config.reg_lambda,
    ...
    # Extract confusion matrix and remove it from final_metrics since it's not a float
    conf_mat = final_metrics.pop("confusion_matrix")
    ...
    if req.network_config.hidden_layers == 0 and req.dataset == "xor" and "xor_failure_witnessed" not in insights_collected:
        add_insight(req.session_id, "xor_failure_witnessed")
        new_insights.append("xor_failure_witnessed")
    if req.network_config.regularization != "none" and "regularization_used" not in insights_collected:
